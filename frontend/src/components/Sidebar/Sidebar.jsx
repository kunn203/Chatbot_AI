...
export default function Sidebar({ activeTab, setActiveTab, onNewChat, sessions, activeSessionId, onSelectSession, onDeleteSession, currentUser, isSidebarOpen, setIsSidebarOpen }) {
  ...
  return (
    <>
      ...
      {/* Sidebar container */}
      <aside className={`
        fixed inset-y-0 left-0 z-50 w-64 bg-sidebar-bg border-r border-gray-200 flex flex-col p-6 shrink-0 transition-transform duration-300 ease-in-out
          ...
          {displaySessions.length === 0 ? (
            ...
            <div className="space-y-1">
              {displaySessions.map(session => (
                <div 
                  key={session.id}
                  onClick={() => { onSelectSession(session.id); setIsSidebarOpen(false); }}
